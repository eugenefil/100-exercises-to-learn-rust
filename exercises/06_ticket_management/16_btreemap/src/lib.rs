...
impl TicketStore {
    ...
    pub fn add_ticket(&mut self, ticket: TicketDraft) -> TicketId {
        ...
        assert!(self.tickets.insert(id, ticket).is_none());
        id
    }
...
pub struct TicketIter<'a> {
    iter: btree_map::Iter<'a, TicketId, Ticket>
}
...
impl<'a> IntoIterator for &'a TicketStore {
    type Item = &'a Ticket;
    type IntoIter = TicketIter<'a>;

    fn into_iter(self) -> Self::IntoIter {
        TicketIter { iter: self.tickets.iter() }
    }
}
